...
variable "database_diagnostic_settings" {
  type = map(object({
...
A map of diagnostic settings to create on the Redis Enterprise database. The map key is deliberately arbitrary to avoid issues where map keys may be unknown at plan time.
...
DESCRIPTION
  ...
  validation {
    condition     = alltrue([for _, value in var.database_diagnostic_settings : contains(["Dedicated", "AzureDiagnostics"], value.log_analytics_destination_type)])
    error_message = "Log analytics destination type must be one of: Dedicated, AzureDiagnostics."
  }
  validation {
    condition = alltrue([
      for _, value in var.database_diagnostic_settings :
      value.workspace_resource_id != null || value.storage_account_resource_id != null || value.event_hub_authorization_rule_resource_id != null || value.marketplace_partner_resource_id != null
    ])
...
variable "diagnostic_settings" {
  ...
  default     = {}
  description = <<DESCRIPTION
A map of diagnostic settings to create on the Redis Enterprise cluster. The map key is deliberately arbitrary to avoid issues where map keys may be unknown at plan time.
...
DESCRIPTION
  ...
  validation {
    condition     = alltrue([for _, value in var.diagnostic_settings : contains(["Dedicated", "AzureDiagnostics"], value.log_analytics_destination_type)])
    error_message = "Log analytics destination type must be one of: Dedicated, AzureDiagnostics."
  }
  validation {
    condition = alltrue([
      for _, value in var.diagnostic_settings :
      value.workspace_resource_id != null || value.storage_account_resource_id != null || value.event_hub_authorization_rule_resource_id != null || value.marketplace_partner_resource_id != null
    ])
    error_message = "At least one of `workspace_resource_id`, `storage_account_resource_id`, `marketplace_partner_resource_id`, or `event_hub_authorization_rule_resource_id` must be set."
  }
